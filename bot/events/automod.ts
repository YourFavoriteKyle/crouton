...
import { getUnicodeByEmojiName } from '../utils/functions';
...
const event: BotEvent = {
	...
	execute: async (message: Message) => {
		...
		eventConfig?.conditions.forEach(
			async (condition: GuildEventCondition) => await parseEventCondition(message, condition)
		);
	}
};
	...
	for (const key in keywords) {
		if (keywords[key].split(' ').length == 1) {
			...
			for (const word in content) {
				if (includes(content[word].replace(/[^a-zA-Z ]/g, ''), keywords[key], true)) return true;
			}
		} else {
...
function includes(content: string, keyword: string, exact: boolean = false): boolean {
	if (exact) {
...
function requiredAuthor(message: Message, condition: GuildEventCondition): boolean {
	if (
...
function getEmojiByName(message: Message, emojiName: string): GuildEmoji | string | undefined {
	const guildEmoji = message.guild?.emojis.cache.find((emoji) => emoji.name == emojiName);
